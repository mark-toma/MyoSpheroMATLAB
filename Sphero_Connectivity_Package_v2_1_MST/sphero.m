classdef sphero<handle
  ...
  methods(Static, Hidden)
    function rot = quat2rot(quat)
      % quat2rot(quat)  Convert unit quaternions to rotation matrices
  ...
  methods%(Access = private)
    ...
    function sensorDataStreaming(obj, ~, ~)
      ...
      % handle populating rot_log when all quat elements are streaming
      vals = obj.Api.SensorData;
      if all(isfield(vals,{'Q0','Q1','Q2','Q3'}))
        q = [vals.Q0',vals.Q1',vals.Q2',vals.Q3'];
        q(:,2:4) = -q(:,2:4); % invert
        qN = sqrt(sum(q'.^2))';
        q = q./repmat(qN,[1,4]);
        rot = obj.quat2rot(q);
        obj.rot_log = cat(3,obj.rot_log,rot);
      end
